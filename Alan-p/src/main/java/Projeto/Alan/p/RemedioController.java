...
public class RemedioController {
    ...
    public RemedioController(Application application) {
    ...
    public ResponseEntity<DadosDetalhamentoRemedio> cadastrar(
            @RequestBody @Valid DadosCadastroRemedios dados,
            UriComponentsBuilder uriBuilder) {

        var remedio = new Remedio(dados);
        repository.save(remedio);

        var uri = uriBuilder.path("/remedios/{id}")
    ...
    // Atualizar informações de um remédio
    @PutMapping
    @Transactional
    ...
    // Excluir definitivamente um remédio
    @DeleteMapping("/{id}")
    @Transactional
